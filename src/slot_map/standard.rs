...
impl<T> StandardSlotMap<T> {
    ...
    #[must_use]
    pub const fn head(&self) -> NonMaxU32 {
        self.head
    }

    #[must_use]
    pub const fn tail(&self) -> NonMaxU32 {
        self.tail
    }

    #[must_use]
    pub const fn free_head(&self) -> NonMaxU32 {
        self.free_head
    }
}

impl<T> Default for StandardSlotMap<T> {
    fn default() -> Self {
        Self {
            head: NonMaxU32::new_none(),
            tail: NonMaxU32::new_none(),
...
impl<T> SlotMap for StandardSlotMap<T> {
    type Id = OrderIdU32;
    type Data = T;
...
impl<T: PartialEq> TestableSlotMap for StandardSlotMap<T> {
    type Data = T;
    type Utype = u32;
