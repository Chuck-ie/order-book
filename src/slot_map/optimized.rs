...
impl<T> Default for OptimizedSlotMap<T> {
    fn default() -> Self {
        Self {
            head: NonMaxU32::new_none(),
            tail: NonMaxU32::new_none(),
...
impl<T> SlotMap for OptimizedSlotMap<T> {
    type Id = OrderIdU32;
    type Data = T;
...
impl<T: PartialEq> TestableSlotMap for OptimizedSlotMap<T> {
    type Data = T;
    type Utype = u32;
...
impl<'a, T> IntoIterator for &'a OptimizedSlotMap<T> {
    type Item = &'a T;
...
impl<'a, T> IntoIterator for &'a mut OptimizedSlotMap<T> {
    type Item = &'a mut T;
