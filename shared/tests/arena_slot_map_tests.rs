...
mod tests {
    use shared::{
        final_ver::{
            ...
            arena_slot_map::ArenaSlotMap,
        },
        slot_map::NonMaxU32,
    };

    fn get_test_arena(chunk_count: usize, chunk_size: usize) -> ArenaSlotAllocator<u32> {
        ArenaSlotAllocator::new(chunk_count, chunk_size)
    }

    fn get_test_slot_map(arena: &mut ArenaSlotAllocator<u32>) -> ArenaSlotMap<u32> {
        ArenaSlotMap::from_arena(arena)
    }

    fn setup(
        chunk_count: usize,
        chunk_size: usize,
    ) -> (ArenaSlotAllocator<u32>, ArenaSlotMap<u32>) {
    ...
    fn test_insert_multiple_multiple_chunks() {
        ...
        let mut slot_map2 = ArenaSlotMap::from_arena(&mut arena);
